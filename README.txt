...
Prepare the data:
    ...
    Place the background images to be used for expansion in the .\data\background_data folder.
...
1.准备数据：
    ...
    将需要用于扩充的背景图像放置于 .\data\background_data 文件夹中。

2.运行提取脚本：

    运行 1_Extract_flood_areas.py，提取洪水区域并输出结果图像。

3.运行扩展脚本：

    运行 2_Expand_extracted_flow.py，对已提取的洪水区域进行形状调整（reshape）操作。

4.生成最终图像：

    运行 3_Paste_images.py，将处理后的洪水区域与背景图像合成，最终结果将保存于 .\data\finally_flow_data 目录中。
